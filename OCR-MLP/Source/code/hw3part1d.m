...
legend('h=2', 'h=4', 'h=8', 'h=16', 'h=32', 'h=64', 'h=128', 'Location', 'NorthOutside', ...
    'Orientation', 'horizontal');
title('Training Errors vs Epochs');
figure;
for i=1:7
    subplot(2,4,i) ;
    plot(1:nEpochs, trainerror_to_plot(i,:), 'b:', 'LineWidth', 2);
    xlim([0 1000]);
    ylim([0 1.2]);
    frac = strcat('For H = ',num2str(2^i));
    title(frac);
end
